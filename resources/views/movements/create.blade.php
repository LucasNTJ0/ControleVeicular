...
<body>
    ...
    <main class="w-full max-w-sm mx-auto flex flex-col items-start gap-2">
        ...
        <div class="form-containerfull">
            <form action="{{ route('movements.store') }}" method="POST" novalidate>
                    ...
                    <select name="reason_id" id="reason" class="form-select">
                        ...
                        @foreach ($reasons as $reason)
                            <option value="{{ $reason->id }}"
                                {{ old('reason_id') == $reason->id ? 'selected' : '' }}>
                                {{ $reason->descricao }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="div-form">
                    ...
                    <input type="datetime-local" name="estimativa_retorno" id="estimativa_retorno"
                        class="form-select @error('estimativa_retorno') !border-red-600 @enderror"
